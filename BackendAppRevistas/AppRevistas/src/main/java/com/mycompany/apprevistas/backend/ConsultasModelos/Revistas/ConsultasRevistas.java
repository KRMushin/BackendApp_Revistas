    ...
    public Revista publicarRevista(Revista revista){
        try(Connection conn = ConexionBaseDatos.getInstance().getConnection()) {
             ...
             try {
                   ...
                   revista.setCostoBloqueosAnuncios(repositorioGlobales.obtenerPrecioModelo("BLOQUEO_ANUNCIO"));
                   // guardar los datos de la revista
                   Revista revistaGuardada = repositorioRevistas.guardar(revista);
                   // guardar los estados como likes etc
                   repositorioConfiguraciones.guardarEstadosRevista(revistaGuardada.getIdRevista());
                   //guardar las las relaciones con categorias etiquetas etc
                   repositorioRelacionRevista.guardarEtiquetas(revistaGuardada.getIdRevista(), revista.getIdEtiquetas());
                   repositorioRelacionRevista.agregarCategoriaRevista(revistaGuardada.getIdRevista(), revista.getIdCategoria());
                   conn.commit();
