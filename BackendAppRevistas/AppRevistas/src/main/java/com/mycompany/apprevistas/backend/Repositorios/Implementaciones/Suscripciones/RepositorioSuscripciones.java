...
public class RepositorioSuscripciones {
    ...
    public List<LlaveRevistaDTO> obtnerRevistasSuscritas(String nombreUsuario) throws SQLException {
        List<LlaveRevistaDTO> revs = new ArrayList<>();

        String insertQuery = "SELECT c.id_revista, c.revista_comentable, c.revista_likeable, c.anuncios_bloqueados,r.titulo_revista, r.descripcion "
                + "FROM suscripciones s JOIN configuracion_revistas c ON s.id_revista = c.id_revista JOIN revistas r ON c.id_revista = r.id_revista WHERE s.nombre_usuario = ?";
        try(PreparedStatement stmt = conn.prepareStatement(insertQuery)) {
             stmt.setString(1, nombreUsuario);
             ResultSet rs = stmt.executeQuery();
             
             while (rs.next()) {
                  revs.add(obtenerLlaveRevistaDTO(rs));
            }
        }
        return revs;
    }
    ...
    public Suscripcion guardarSuscripcion(Suscripcion suscripcion) throws SQLException{
        ...
        try(PreparedStatement stmt = conn.prepareStatement(insertUpdate, PreparedStatement.RETURN_GENERATED_KEYS)){
            ...
            stmt.setString(1, suscripcion.getNombreUsuario());
             ...
             if (filasAfectadas == 0) {
                 throw new DatosInvalidosUsuarioException("verifique los datos de suscripcion");
            }
    ...
    private LlaveRevistaDTO obtenerLlaveRevistaDTO(ResultSet rs) throws SQLException {
        LlaveRevistaDTO revistaLlave = new LlaveRevistaDTO();
        revistaLlave.setIdRevista(rs.getLong("id_revista"));
        revistaLlave.setRevistaComentable(rs.getBoolean("revista_comentable"));
        revistaLlave.setRevistaLikeable(rs.getBoolean("revista_likeable"));
        revistaLlave.setTituloRevista(rs.getString("titulo_revista"));
        revistaLlave.setDescripcion(rs.getString("descripcion"));
        return revistaLlave;
    }
...
//
//        String updateLikes = "UPDATE revistas r JOIN suscripciones s ON r.id_revista = s.id_revista SET r.numero_likes = r.numero_likes + 1, s.aprecio_suscripcion = TRUE "
//                                            + "WHERE s.nombre_usuario = ? AND r.id_revista = ? AND s.aprecio_suscripcion = FALSE;";
